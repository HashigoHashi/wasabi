...
pub enum TranslationResult {
    PageMapper4K{ phys: u64},
    PageMapper2M{ phys: u64},
    PageMapper1G{ phys: u64},
}
...
pub unsafe fn write_cs(cs: u16) {
    // The MOV instruction CANNNOT be used to load the CS register.
...
/// # Safety
/// Anything can happen if the given selector si invalid.
...
/// # Safety
/// Anything can happen if the given selctor is invalid.
...
struct InterruptInfo {
    // This suruct is placed at top of the interrupt stack.
...
/// This geferates interrupt_entrypointN()
/// Geverated asm will be looks like this:
/// ```
...
inthandler_common:
    ...
    sub rsp, 512 + 8
    fxsave64[rsp]
    // 1st parametr: pointer to the saved CPU state
    mov rdi, rsp
...
#[no_mangle]
extern "sysv64" fn inhandler(info: &InterruptInfo, index: usize) {
...
enum IdtAttr {
    ...
    // thie struct will be undefined behavior.
    _NotPresent = 0,
